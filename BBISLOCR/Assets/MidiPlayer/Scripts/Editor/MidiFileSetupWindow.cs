    ...
    public class MidiFileSetupWindow : EditorWindow
    {
        ...
        private void ShowListMidiFiles(float startX, float startY, float width, float height)
        {
            try
            {
                if (columnSF == null)
                {
                    columnSF = new ToolsEditor.DefineColumn[4];
                    columnSF[0].Width = 40; columnSF[0].Caption = "Index"; columnSF[0].PositionCaption = 5f;
                ...
                GUIContent content = new GUIContent()
                {
                    text = MidiPlayerGlobal.CurrentMidiSet.MidiFiles == null || MidiPlayerGlobal.CurrentMidiSet.MidiFiles.Count == 0 ?
                                        "No Midi file available yet" : "Midi files available",
                    tooltip = ""
                };
                ...
                if (GUI.Button(new Rect(startX + localstartX + espace, startY + localstartY, buttonLargeWidth, buttonHeight), "Add a Midi file"))
                    AddMidifile();

                if (GUI.Button(new Rect(startX + localstartX + 2 * espace + buttonLargeWidth, startY + localstartY, buttonLargeWidth, buttonHeight), "Add Midis from a folder"))
                    AddMidiFromFolder();
                ...
                if (MidiPlayerGlobal.CurrentMidiSet.MidiFiles != null)
                {
                    ...
                    Rect listContentRect = new Rect(0, 0, width - 25, MidiPlayerGlobal.CurrentMidiSet.MidiFiles.Count * itemHeight + 5);

                    scrollPosMidiFile = GUI.BeginScrollView(listVisibleRect, scrollPosMidiFile, listContentRect, false, true);
                    float boxY = 0;
                    ...
                    for (int i = 0; i < MidiPlayerGlobal.CurrentMidiSet.MidiFiles.Count; i++)
                    {
                        ...
                        // Draw a row
                        GUI.Box(new Rect(espace, boxY, width - 35, itemHeight), "", indexEditItem == i ? styleListRowSelected : styleListRow);

                        // col 0 - Index
                        float colw = columnSF[0].Width;
                        EditorGUI.LabelField(new Rect(boxX + 1, boxY + 2, colw, itemHeight - 5), i.ToString(), styleLabelRight);
                        boxX += colw;
        ...
        private static void InsertMidiFIle(string selectedFile)
        {
            ...
            Debug.Log($"Load Midi file {selectedFile} with success, duration: {midifile.MPTK_DurationMS / 1000f} sec.");
